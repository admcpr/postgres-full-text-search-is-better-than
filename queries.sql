/* Naive title queries */
select title from assets where title like '%star wars%' -- :( No results

select title from assets where title like '%star%' or title like '%wars%' -- :( More results, but still not Star Wars ??

-- Oh no like is case sensitive
select title from assets where title ilike '%star%' or title ilike '%wars%' -- :( Even more results, and Star Wars but where is it?
...
select to_tsvector(title), title from assets limit 20;
...
--
-- alter table assets add title_search tsvector;
-- update assets set title_search = 
...
-- alter table assets drop title_search;
alter table assets 
...
create index idx_search on assets using GIN(title_search)
